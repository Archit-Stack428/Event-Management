...
$stmt = $conn->prepare("SELECT event_title, event_desc FROM create_event WHERE event_id = ?");
$stmt->bind_param('i', $eventid);
$stmt->execute();
$result = $stmt->get_result();

while($row = $result->fetch_assoc()) {
   echo "<h4 class='card-title'>".htmlspecialchars($row['event_title'])."</h4>";
   echo '<h6 class="text-muted card-subtitle mb-2">Description</h6><p class="card-text">'.htmlspecialchars($row['event_desc']).'</p><div style="width: 100%;padding-bottom: 10px;">';
}       
$stmt->close();
